...
from XPolicyLab.policy.Pi_05.manual_control import ManualControlWebServer, run_policy_and_control
...
def main(deploy_cfg):
    ...
    if protocol == "ws":
        ...
        control_server = None
        if getattr(model, "control", None) is not None and deploy_cfg.get("control_ui_enabled", True):
            control_server = ManualControlWebServer(
                model.control,
                host=str(deploy_cfg.get("control_ui_host", "127.0.0.1")),
                port=int(deploy_cfg.get("control_ui_port", 18080)),
                token=os.environ.get("PI05_CONTROL_TOKEN"),
                require_token=bool(deploy_cfg.get("control_ui_require_token", True)),
            )
        try:
            if control_server is None:
                asyncio.run(server.serve_forever())
            else:
                asyncio.run(run_policy_and_control(server, control_server))
