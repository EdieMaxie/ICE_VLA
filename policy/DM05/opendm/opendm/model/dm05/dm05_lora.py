...
def _dump_trainable_summary(
    ...
    allowed_markers = tuple(
        ...
        + (["action_expert"] if "full_action" in lora_config.style else [])
    )
...
def apply_lora_to_dm05_model(
    ...
    if "full_action" in lora_config.style:
        # PI05-style hybrid fine-tuning: keep the entire DM05 action path
        # trainable while PEFT owns only the VLM LoRA adapters. The action
        # expert has a keyword-only forward, so it cannot be wrapped by
        # PEFT's ModulesToSaveWrapper; unfreeze its original parameters here.
        dm05_model = unwrap_dm05_model(model)
        for parameter in dm05_model.model.action_expert.parameters():
            parameter.requires_grad = True
        logger.info(
            "Hybrid DM05 training: unfreezed full action_expert parameters "
            "outside PEFT ModulesToSaveWrapper."
        )
    logger.info(
