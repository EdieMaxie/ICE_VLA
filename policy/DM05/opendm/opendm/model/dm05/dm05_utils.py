...
def linear_fp32(x: torch.Tensor, linear: torch.nn.Module) -> torch.Tensor:
    ...
    with torch.autocast(device_type=x.device.type, enabled=False):
        ...
        # PEFT LoRA wrappers keep the base/adapters in BF16. The surrounding
        # DM05 path intentionally enters this helper with FP32 inputs, so cast
        # only the wrapper input back to its base-layer dtype before dispatch.
        base_layer = getattr(linear, "base_layer", None)
        base_weight = getattr(base_layer, "weight", None)
        if base_weight is not None:
            return linear(x.to(dtype=base_weight.dtype))
        return linear(x)
