...
class DMTrainer(Trainer):
    ...
    def _configure_fsdp_auto_wrap(self) -> None:
        # PEFT wraps DM05 in PeftModel. Resolve the underlying DM05 model so
        # the explicit action-expert/VLM layer boundaries still FULL_SHARD.
        is_lora = bool(getattr(self.exp_config, "use_lora", False))
        if (
            self.exp_config.model_config.precision_policy != FP32_MIXED_PRECISION_POLICY
            and not is_lora
        ):
            return
        fsdp_plugin = getattr(self.accelerator.state, "fsdp_plugin", None)
        ...
        fsdp_model = self.model.get_base_model() if is_lora else self.model
        modules = fsdp_model.fsdp_wrap_modules()
    ...
    def _link_exp_config(self) -> TrainingArguments:
        ...
        if not getattr(self.exp_config, "use_lora", False):
            # Full-parameter SFT on 4x48-GB GPUs needs compressed Adam states;
            # LoRA follows the upstream SO101 recipe and uses regular AdamW.
            linked_args["optim"] = "adamw_bnb_8bit"
        if self.exp_config.model_config.precision_policy == FP32_MIXED_PRECISION_POLICY:
            linked_args.update(
                {
                    "bf16": True,
        ...
        tensorboard_log_dir = getattr(
            self.exp_config.trainer_config, "tensorboard_log_dir", None
        )
        wandb_project = self.exp_config.trainer_config.wandb_project
        if tensorboard_log_dir:
            linked_args["report_to"] = ["tensorboard"]
            linked_args["logging_dir"] = tensorboard_log_dir
        elif wandb_project:
        ...
        if self.exp_config.trainer_config.fsdp1 and world_size > 1:
            # Four 48-GB GPUs cannot hold replicated full parameters together
            # with AdamW states after the first optimizer step. FULL_SHARD keeps
            # parameters, gradients, and optimizer state distributed.
            linked_args["fsdp"] = "full_shard"
